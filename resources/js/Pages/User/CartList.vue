...
<template>
  ...
  <div class="container px-5 py-24 mx-auto flex sm:flex-nowrap flex-wrap" bis_skin_checked="1">
...
<div>
    <input type="number" id="first_product_{{product.id}}" 
...
<button @click.prevent="update(product, carts[itemId(product.id)].quantity + 1)"
    ...
    <div class="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0" bis_skin_checked="1">
      <h2 class="text-gray-900 text-lg mb-1 font-medium title-font">Summary</h2>
      <p class="leading-relaxed mb-5 text-gray-600">Total: Rp.{{ total }}</p>

      <h2 class="text-gray-900 text-lg mb-1 font-medium title-font">Address</h2>
      ...
      <button class="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg">Booking</button>
      <p class="text-xs text-gray-500 mt-3">Continue Book</p>
    </div>
  </div>
</section>
    </UserLayouts>
